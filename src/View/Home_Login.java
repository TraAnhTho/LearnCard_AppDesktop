...
public class Home_Login extends JFrame {
	...
	public Home_Login(String iduser) throws Exception {
		...
		// Table EditList
		String[] header = { "STT", "Tên danh sách", "List", "User" };
		...
		// Table card
		String[] header3 = { "STT", "Tên danh sách", "Khái niệm", "Định Nghĩa" };
...
//		       LoadDBData3JTable();
		...
		// Table DeleteList
		String[] header1 = { "STT", "Tên danh sách", "List", "User" };
		...
		// Table card
		String[] header4 = { "STT", "Tên danh sách", "Khái Niệm", "Định Nghĩa" };
...
//					dispose();
	...
	public void LoadDBData2JTable() throws Exception {
		...
		String sql = "select * from flash_card.list_card Where usercol='" + iduser + "';";
	...
	public void LoadDBData3JTable(String listname) throws Exception {
		try {
			...
			String sql = "select * from flash_card.card where cardcol='" + listname + "' AND usercol ='"
					+ iduser + "';";
	...
	public void InsertsCardEnd() {
		if (textField_card.getText().trim().isEmpty() || textField_des.getText().trim().isEmpty()) {
			...
			try {
				dispose();
				JOptionPane.showMessageDialog(new Home_Login(iduser),
						"Đã lưu thành công bộ card: " + txt_list_card.getText(), "Đây là cửa số thông báo",
